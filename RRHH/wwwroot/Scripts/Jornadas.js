...
var jornadaModal = ComponentBuilder.setModal("modal_Jornadas");
...
function obtenerDatosValidos(idEmpre) {
    ...
    $.get("ListarJornadas?empresa=" + idEmpre, function (data) {

        if (data.info.result == 0) {
            alert(data.info.mensaje);
            document.getElementById("esperar").style.display = 'none';
        } else {
            crearListado(["Codígo", "Descripción", "Días Trab.", "Días Desc.", "Días Tot.", "Ciclos", "Horas Sem."], data, "list_jornadas");
    ...
    if (z.data.length > 0) {
        ...
        for (var i = 0; i < z.data.length; i++) {
            ...
            for (var j = 0; j < propiedadesObjeto.length; j++) {
                ...
                else if (nombrePropiedad == 'diasTrabajo') {
                    contenido += "<td>" + fila[nombrePropiedad] + "</td>";
                }
                else if (nombrePropiedad == 'diasDescanso') {
                    contenido += "<td>" + fila[nombrePropiedad] + "</td>";
                }
                else if (nombrePropiedad == 'diasTotales') {
                    contenido += "<td>" + fila[nombrePropiedad] + "</td>";
                }
                else if (nombrePropiedad == 'numeroCiclos') {
                    contenido += "<td>" + fila[nombrePropiedad] + "</td>";
                }
                else if (nombrePropiedad == 'horasSemanales') {
                    contenido += "<td>" + fila[nombrePropiedad] + "</td>";
                }
            };
            ...
            contenido += "data-bs-toggle='modal' data-bs-target='#modal_Jornadas'><i class='fa fa-pen'></i></button> ";
...
function abrirModal(id) {
    ...
    if (id != undefined) {
        ...
        document.getElementById("txt_Cabecera").innerHTML = "Editar jornada";

        $.get("ConsultaJornadaId?id=" + id + "&empresa=" + idEmpre, function (data) {
            data = data.info.data;
            console.log(data);
            ...
            document.getElementById("txt_trabajo").value = data[0].diasTrabajo;
            document.getElementById("txt_descanso").value = data[0].diasDescanso;
            document.getElementById("txt_ciclos").value = data[0].numeroCiclos;
            document.getElementById("txt_horas").value = data[0].horasSemanales;
            document.getElementById("txt_resolucion").value = data[0].resolucion;
            document.getElementById("txt_fecres").value = data[0].fechaResolucion;
        ...
        document.getElementById("txt_Cabecera").innerHTML = "Nueva jornada";
...
function Eliminar(id) {
    ...
    if (confirm("confirma que desea eliminar la información de esta jornada?") == 1) {

        $.get("InhabilitaJornada?Id=" + id + "&empresa=" + idEmpre, function (data) {
            if (data.info.result == 0) {
...
function Guarda() {
    ...
    var trabajo = document.getElementById("txt_trabajo").value.toString().toUpperCase();
    var descanso = document.getElementById("txt_descanso").value;
    var ciclos = document.getElementById("txt_ciclos").value;
    var horas = document.getElementById("txt_horas").value;
    var resolucion = document.getElementById("txt_resolucion").value;
    var fecres = document.getElementById("txt_fecres").value;
    var empresa = idEmpre;
    ...
    frm.append("diasTrabajo", trabajo);
    frm.append("diasDescanso", descanso);
    frm.append("numeroCiclos", ciclos);
    frm.append("horasSemanales", horas);
    frm.append("resolucion", resolucion);
    frm.append("fechaResolucion", fecres);
