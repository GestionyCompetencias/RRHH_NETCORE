...
var descuentosinformadosModal = ComponentBuilder.setModal("modal_Descuentosinformados");
...
function mostrarDatos() {
    ...
    $.get("EmpresaLog", function (data) {
        idEmpre = data;
        codigo = document.getElementById("cbo_descuentos").value
      if (codigo != "") {
...
function obtenerDatosValidos(idEmpre) {
    ...
    $.get("ListarDescuentosInformados?codigo=" + codigo + '&mes=' + mes + '&anio=' + anio + '&pago=' + pago, function (data) {
        document.getElementById("nuevo").style.display = 'block';
        if (data.info.result == 0) {
            alert(data.info.mensaje);
        }
            crearListado(["Rut", "Nombre", "Afecta","pago", "Calculo","Monto", "Desde", "Hasta"], data, "list_descuentosinformados");
            document.getElementById("esperar").style.display = 'none';
    })
...
function abrirModal(id) {
    limpiar();
    document.getElementById("cbo_descuentos").value = codigo;

    document.getElementById("divErrores").innerHTML = "";

    if (id != undefined) {

        document.getElementById("txt_Cabecera").innerHTML = "Editar descuentos informados";

        $.get("ConsultarDescuentoInformadoId?id="+ id +"&empresa="+idEmpre, function (data) {
            data = data.info.data;
        ...
        document.getElementById("txt_Cabecera").innerHTML = "Nuevo descuento informado";
...
function Eliminar(id) {
    ...
    if (confirm("confirma que desea eliminar la información del descuento?") == 1) {

        $.get("inhabilitaDescuentoInformado?Id=" + id + "&empresa=" + idEmpre, function (data) {
                alert(data.info.mensaje);
                inicio();
        })
...
function ComboDescuentos() {
    $.get("CargaDescuentos?empresa=" + idEmpre, (data) => {
       ...
       var contenido = "";
        ...
        document.getElementById("cbo_descuentos").innerHTML = contenido;
    })
}
