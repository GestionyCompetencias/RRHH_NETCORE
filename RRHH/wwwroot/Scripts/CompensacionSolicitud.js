...
function ObtenerDatos() {
    ...
    dias = document.getElementById("txt_dias").value;
    if (rut != "") {
        $.get("VerificaCompensacionSolicitud/?rut=" + rut + '&dias=' + dias, function (data) {
...
function MostrarDatos() {
    ...
    if (rut != "") {
            //document.getElementById("esperar").style.display = 'block';
            $.get("ConsultarCompensacionSolicitud/?rut=" + rut + '&dias=' + dias, function (data) {
                ListaRegistros(["Periodo", "Desde", "Habiles", "Corridos"], data, "list_compensacionsolicitud");
    ...
    if (data.info.result ==1) {
        ...
        for (var i = 0; i < info.length; i++) {
            ...
            for (var j = 0; j < propiedadesObjeto.length; j++) {
                ...
                if (nombrePropiedad == "periodo" || nombrePropiedad == "fechainicio" || nombrePropiedad == "diascorridos" || nombrePropiedad == "diastotal")
...
function Procesar() {
    ...
    if (rut != "") {
        //document.getElementById("esperar").style.display = 'block';
        $.get("ProcesarCompensacionSolicitud/?rut=" + rut + '&dias=' + dias, function (data) {
            alert(data.info.mensaje);
            //document.getElementById("esperar").style.display = 'none';
