...
$(function () {
    ...
    $(document).on("click", "#btn-filter", function (e) {
        ...
        const year = $("#anio").val();
        if (month > 0) {
            const dest = `ListarCuadraturaDescuentos?mes=${month}&anio=${year}`;
            ...
            $.get(dest, (resp) => {
                ...
                if (libro !== null) {
                    ...
                    const cabeceras = ['Codigo', 'Descripción', 'Cantidad', 'Monto'];
                    ...
                    document.getElementById("list_cuadraturadescuentos").innerHTML = '';
                    ...
                    $("#list_cuadraturadescuentos").html(msg);
                }
    ...
    $(document).on("click", "#export", function () {
        $("#table-libroZ").table2excel({
            filename: "CuadraturaDescuento.xls",
        })
    });
...
const cargarTabla = (cabeceras, data) => {
    ...
    data.forEach(lib => {
        ...
        lib.forEach(itLib => {
            contenido += `<tr>`;
            contenido += `<td>${itLib.descuento}</td>`;
    ...
    $("#list_cuadraturadescuentos").html(contenido);
}
...
const cargarTablaImpresion = (cabeceras, data, mes, anio) => {
    ...
    $("#title-impresion").html(`CUADRATURA DE DESCUENTOS DE ${meses[mes]} DEL A&Ntilde;O ${anio}`);
    ...
    data.forEach(lib => {
        ...
        lib.forEach(itLib => {
            contenido += `<tr>`;
            contenido += `<td class="text-right">${itLib.descuento}</td>`;
            contenido += `<td class="text-right">${itLib.descripcion}</td>`;
            contenido += `<td class="text-right" align="right">${formatNumber(itLib.cantidad)}</td>`;
            contenido += `<td class="text-right" align="right">${formatNumber(itLib.monto)}</td>`;
            contenido += `</tr>`;
