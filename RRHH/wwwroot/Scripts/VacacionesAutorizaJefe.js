...
function obtenerDatosValidos() {
    ...
    $.get("ListarVacacionesAutorizaJefe?desde=" + des +"&hasta="+has , function (data) {
        if (data.info.result == 0) {
            alert(data.info.mensaje);
        }
        crearListado(["Rut", "Nombre", "Inicio", "Termino", "Dias","Autoriza","Rechaza"], data, "list_autorizavacacionesjefe");
        document.getElementById("esperar").style.display = 'none';
    })
...
function Autoriza() {
    ...
    if (nelementos > 0) {
        ...
        if (ids != "") {
            $.get("AutorizaVacaciones/?ids=" + ids + '&des=' + des + '&has=' + has, function (data) {
...
function Rechaza() {
    ...
    if (nelementos > 0) {
        ...
        if (ids != "") {
            $.get("RechazaVacaciones/?ids=" + ids + '&des=' + des + '&has=' + has, function (data) {
                if (data.info.result == 1)
                alert(data.info.mensaje);
            })
        }
