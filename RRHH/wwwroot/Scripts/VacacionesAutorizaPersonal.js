...
function obtenerDatosValidos() {
    ...
    $.get("ListarVacacionesAutorizaPersonal?desde=" + des +"&hasta="+has , function (data) {
        if (data.info.result == 0) {
            alert(data.info.mensaje);
        }
        crearListado(["Rut", "Nombre", "Inicio", "Termino", "Dias","Registra","Rechaza"], data, "list_autorizavacacionespersonal");
        document.getElementById("esperar").style.display = 'none';
    })
    ...
    if (data.info.result != 0) {
        ...
        for (var i = 0; i < z.data.length; i++) {
            ...
            for (var j = 0; j < propiedadesObjeto.length; j++) {
                ...
                else if (nombrePropiedad == 'diastotal') {
                    contenido += "<td>" + fila[nombrePropiedad] + "</td>";
                }
            };
...
function Procesar() {
    if (confirm("Confirma que desea procesar vacaciones") == 1) {
        Autoriza();
        Rechaza();
        mostrarDatos();
    }
...
function Autoriza() {
    ...
    if (nelementos > 0) {
        ...
        if (ids != "") {
            $.get("AutorizaVacacionesPersonal/?ids=" + ids + '&des=' + des + '&has=' + has, function (data) {
...
function Rechaza() {
    ...
    if (nelementos > 0) {
        ...
        if (ids != "") {
            $.get("RechazaVacacionesPersonal/?ids=" + ids + '&des=' + des + '&has=' + has, function (data) {
                if (data.info.result == 1)
                alert(data.info.mensaje);
            })
        }
