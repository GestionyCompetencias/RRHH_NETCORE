...
$(function () {
    ...
    $(document).on("click", "#btn-filter", function (e) {
        ...
        const dest = `ListarPermisosConsulta?desde=${des}&hasta=${has}`;
        ...
        $.get(dest, (resp) => {
            ...
            if (libro !== null) {
                ...
                const cabeceras = ['Rut', 'Nombre', 'Inicio', 'hora', 'Termino', 'hora', 'Gose','Dias', 'Comentario'];
                ...
                document.getElementById("list_consultapermisos").innerHTML = '';
                ...
                $("#list_consultapermisos").html(msg);
            }
...
const cargarTabla = (cabeceras, data) => {
    ...
    data.forEach(lib => {
        ...
        contenido += `<td width="100" class="text-right" align="right">${cabeceras[8]}</td>`;
        contenido += `</tr>`;
        contenido += `</thead>`;
        contenido += `<tbody>`;
        lib.forEach(itLib => {
            ...
            contenido += `<td class="text-right" align="right">${itLib.horainicio}</td>`;
            contenido += `<td class="text-right" align="right">${itLib.fechatermino}</td>`;
            contenido += `<td class="text-right" align="right">${itLib.horatermino}</td>`;
            contenido += `<td class="text-right" align="right">${itLib.gosestr}</td>`;
        ...
        contenido += `<td class="bg-light" colspan="7">Totales... </td>`;
    ...
    $("#list_consultapermisos").html(contenido);
}
...
const cargarTablaImpresion = (cabeceras, data) => {
    ...
    $("#title-impresion").html(`Consulta de permisos ${des} hasta ${has}`);
    ...
    contenido += `<th width="100" class="text-right" align="right">${cabeceras[8]}</th>`;
    contenido += `</tr>`;
    contenido += `</thead>`;
