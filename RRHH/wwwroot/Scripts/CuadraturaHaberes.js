...
$(function () {
    ...
    $(document).on("click", "#btn-filter", function (e) {
        ...
        const year = $("#anio").val();
        if (month > 0) {
            const dest = `ListarCuadraturaHaberes?mes=${month}&anio=${year}`;
            ...
            $.get(dest, (resp) => {
                ...
                if (libro !== null) {
                    ...
                    const cabeceras = ['Codigo', 'Descripción', 'Cantidad', 'Monto', '% de sueldo', 'UFs'];
                    ...
                    document.getElementById("list_cuadraturahaberes").innerHTML = '';
                    ...
                    $("#list_cuadraturahaberes").html(msg);
                }
...
const calcularTotalesLibos = (libro) => {
    ...
    libro.forEach(it => {
        let montoCanti = parseFloat(it.cantidad);
        totalCanti = totalCanti + montoCanti;
        let montoMonto = parseFloat(it.monto);
        totalMonto = totalMonto + montoMonto;
        let montoPorce = parseFloat(it.porcentaje);
        totalPorce = totalPorce + montoPorce;
        let montoUefes = parseFloat(it.uefes);
        totalUefes = totalUefes + montoUefes;
    });
...
const cargarTabla = (cabeceras, data) => {
    ...
    data.forEach(lib => {
        ...
        lib.forEach(itLib => {
            contenido += `<tr>`;
            contenido += `<td>${itLib.haber}</td>`;
            contenido += `<td class="text-right">${itLib.descripcion}</td>`;
            contenido += `<td class="text-right" align="right">${formatNumber(itLib.cantidad)}</td>`;
            contenido += `<td class="text-right" align="right">${formatNumber(itLib.monto)}</td>`;
            contenido += `<td class="text-right" align="right">${formatNumber(itLib.porcentaje)}</td>`;
            contenido += `<td class="text-right" align="right">${formatNumber(itLib.uefes)}</td>`;
            contenido += `</tr>`;
        });
        contenido += `</tbody>`;
        contenido += `<tfoot>`;
        ...
        contenido += `<td class="text-right" align="right">${formatNumber(result.totalCanti)}</td>`;
        contenido += `<td class="text-right" align="right">${formatNumber(result.totalMonto)}</td>`;
        contenido += `<td class="text-right" align="right">${formatNumber(result.totalPorce)}</td>`;
        contenido += `<td class="text-right" align="right">${formatNumber(result.totalUefes)}</td>`;
        contenido += `</tr>`;
        contenido += `</tfoot>`;
        contenido += `</table>`;
        i++;
    ...
    $("#list_cuadraturahaberes").html(contenido);
}
...
const cargarTablaImpresion = (cabeceras, data, mes, anio) => {
    ...
    $("#title-impresion").html(`CUADRATURA DE HABERES DE ${meses[mes]} DEL A&Ntilde;O ${anio}`);
