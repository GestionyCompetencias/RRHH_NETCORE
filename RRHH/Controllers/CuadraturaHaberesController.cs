using RRHH.BaseDatos;
using RRHH.Models.Estructuras.Template.Componentes;
using RRHH.Models.ViewModels;
using RRHH.Servicios;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.Filters;
using System.Data;
using RRHH.Servicios.CuadraturaHaberes;

namespace RRHH.Controllers
{
    public class CuadraturaHaberesController : Controller
    {
        private Funciones f = new Funciones();
        private Correos correos = new Correos();
        private Seguridad seguridad = new Seguridad();
        private Generales Generales = new Generales();
        string RutEmpresa = "";
        private readonly ICuadraturaHaberesService _CuadraturaHaberesService;


        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ViewData["breadcrumb"] = new Breadcrumb("Remuneraciones", "Información mensual");
        }

        public CuadraturaHaberesController(ICuadraturaHaberesService CuadraturaHaberesService)
        {
            _CuadraturaHaberesService = CuadraturaHaberesService;
        }

        public IActionResult Index()
        {
            string UsuarioLogeado = HttpContext.Session.GetString("UserIdLog");
            if (seguridad.ValidarUsuario(UsuarioLogeado) == false) return RedirectToAction("Index", "Login");
            return View();
        }

        [HttpGet]
        [Route("ListarCuadraturaHaberes")]
        public async Task<JsonResult> ListarCuadraturaHaberes(int mes, int anio)
        {
            string empresastr = HttpContext.Session.GetString("EmpresaLog");
            int empresa = int.Parse(empresastr);
            Resultado resultado = new Resultado();
            resultado.result = 0;
            resultado.mensaje = "No existen haberes informados";
            resultado.result = 0;
            string pago = "L";
            List<ResCuadraturaHaberes> CuadraturaHaberes = _CuadraturaHaberesService.ListarCuadraturaHaberesService(empresa, mes, anio, pago);
            if (CuadraturaHaberes != null)
            {
                resultado.data = CuadraturaHaberes;
                resultado.mensaje = "Si existen haberes informados";
                resultado.result = 1;
            }
            return Json(new { info = resultado });
        }

    }
}
